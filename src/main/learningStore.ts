...
export function createLearningStore(options: LearningStoreOptions): LearningStore {
  ...
  const state = {
    ...
    topicStatus: loadProgress(options.progressPath, options.seedData)
  };
  ...
  return {
    ...
    updateTopicStatus: (topicId: string, status: string) => {
      ...
      if (!findTopic(state.seedData, topicId)) {
...
function loadProgress(progressPath: string, seedData: SeedData): Record<string, StudyStatus> {
  ...
  try {
    ...
    if (parsed.version !== PROGRESS_VERSION || !parsed.topicStatus || typeof parsed.topicStatus !== 'object') {
      return {};
    }

    const topicIds = new Set(seedData.topics.map((topic) => topic.id));
    return Object.fromEntries(
      Object.entries(parsed.topicStatus).filter(
        ([topicId, status]) => topicIds.has(topicId) && VALID_STATUSES.has(status)
      )
    );
  } catch {
    return {};
